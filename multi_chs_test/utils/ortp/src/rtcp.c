...
mblk_t *sdes_chunk_new(uint32_t ssrc){
...
mblk_t * sdes_chunk_append_item(mblk_t *m, rtcp_sdes_type_t sdes_type, const char *content)
{	
	if ( content )
	{
		sdes_item_t si;
...
void sdes_chunk_set_ssrc(mblk_t *m, uint32_t ssrc){
	sdes_chunk_t *sc=(sdes_chunk_t*)m->b_rptr;
	sc->csrc=htonl(ssrc);
}
...
void report_block_init(report_block_t *b, RtpSession *session){
	...
	b->cum_num_packet_lost=hton24((uint32_t) stream->stats.cum_packet_loss);
