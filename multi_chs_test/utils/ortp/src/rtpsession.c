...
static int rtcp_recv (RtpSession * session);
...
**/
int
rtp_session_send_with_ts (RtpSession * session, const char * buffer, int len,
			  uint32_t userts)
{
	mblk_t *m;
	int err;
#ifdef USE_SENDMSG
	m=rtp_session_create_packet_with_data(session,(char*)buffer,len,NULL);
#else
	m = rtp_session_create_packet(session,RTP_FIXED_HEADER_SIZE,(char*)buffer,len);
#endif
...
static int
rtp_recv (RtpSession * session, uint32_t user_ts)
{
	int error;
	...
	while (1)
	{
		...
		if (error > 0)
		{
			if (error<RTP_FIXED_HEADER_SIZE){
				...
				p = (char *)ortp_realloc (mp->b_wptr, error);
				if (p != mp->b_wptr)
...
rtcp_recv (RtpSession * session)
{
	...
	while (1)
	{
		...
		if (error > 0)
		{
			...
			if (session->rtcp.rem_addrlen==0 && addrlen>0){
...
rtp_session_recvm_with_ts (RtpSession * session, uint32_t user_ts)
{
	...
	if (session->flags & RTP_SESSION_RECV_SYNC)
	{
		...
		if ((uint32_t) (newest->timestamp - oldest->timestamp) <
		    (uint32_t) session->rtp.jittctl.jitt_comp_ts)
		{
